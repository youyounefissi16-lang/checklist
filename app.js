...
(async function init() {
  ...
  if (!currentUser) { currentUser = "Test User"; Storage.setUser(currentUser); }
  currentView = "analytics";
  render();
  // Auto-load test data if no sessions exist (temporary)
  if (!sessions.length) __loadTestData();
})();

// ---- TEMP TEST DATA (remove after testing) ----
window.__loadTestData = function () {
  if (!zones.length) {
    zones.push(
      { zoneId: "test-z1", zoneName: "Kitchen", type: "checklist", items: [
        { id: "test-i1", text: "Temperature check", status: "unchecked", checkedBy: null, checkedAt: null, note: "", noteColor: "red" },
        { id: "test-i2", text: "Hand wash station", status: "unchecked", checkedBy: null, checkedAt: null, note: "", noteColor: "red" },
        { id: "test-i3", text: "Floor cleanliness", status: "unchecked", checkedBy: null, checkedAt: null, note: "", noteColor: "red" }
      ]},
      { zoneId: "test-z2", zoneName: "Storage", type: "checklist", items: [
        { id: "test-i4", text: "Expiration dates", status: "unchecked", checkedBy: null, checkedAt: null, note: "", noteColor: "red" },
        { id: "test-i5", text: "Shelving condition", status: "unchecked", checkedBy: null, checkedAt: null, note: "", noteColor: "red" }
      ]},
      { zoneId: "test-z3", zoneName: "Dining Area", type: "checklist", items: [
        { id: "test-i6", text: "Table sanitation", status: "unchecked", checkedBy: null, checkedAt: null, note: "", noteColor: "red" },
        { id: "test-i7", text: "Chair condition", status: "unchecked", checkedBy: null, checkedAt: null, note: "", noteColor: "red" }
      ]}
    );
  }
  var now = new Date();
  var inspectors = ["Alice", "Bob", "Carlos"];
  for (var i = 0; i < 24; i++) {
    var mo = Math.floor(Math.random() * 12);
    var day = 1 + Math.floor(Math.random() * 27);
    var d = new Date(now.getFullYear(), now.getMonth() - mo, day, 8 + Math.floor(Math.random() * 10), Math.floor(Math.random() * 60));
    var items = [];
    zones.forEach(function (z) {
      z.items.forEach(function (it) {
        var r = Math.random();
        var st = r < 0.55 ? "pass" : (r < 0.82 ? "no_pass" : "unchecked");
        items.push({
          zoneName: z.zoneName, type: "checklist", text: it.text,
          status: st,
          checkedBy: st === "unchecked" ? null : inspectors[Math.floor(Math.random() * inspectors.length)],
          checkedAt: st === "unchecked" ? null : d.toISOString().slice(0, 19).replace("T", " "),
          note: "", noteColor: "red"
        });
      });
    });
    sessions.push({
      id: "test-" + Date.now() + "-" + i + "-" + Math.random().toString(36).slice(2, 6),
  ...
  persist();
  render();
  console.log("Test data loaded: " + sessions.length + " sessions, " + zones.length + " zones");
};
